"""NWS weather-alert sensor (fixture default; optional live leg, user opt-in).

Default poll() serves the bundled CC0 fixture (offline). The live leg
is a pure function over NWS GeoJSON text the *caller* supplies —
...
def from_nws_geojson(text: str, limit: int = 20):
    """Map NWS alert GeoJSON (caller-fetched) to canonical events."""
    from gods_eye.future import world_live as _live
    return _live.parse_nws_alerts(text, limit=limit)
