...
1. **Explicit install only.** A plugin enters the machine only via an
   ...
   `tellurion plugins --dir`). Discovery never downloads, never scans the
...
6. **No secrets, no undisclosed network.** The declaration must state
   `network` and `secrets` needs honestly. Discovery and demo paths make
   no outbound network calls (localhost only).
7. **Licence boundary.** In-process adapters must be permissive
   (MIT/Apache-2.0/BSD). GPL/AGPL code is never linked.
...
`tellurion plugins` reports per plugin: licence, data rights, network and
